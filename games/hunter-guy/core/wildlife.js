import { seededRandom, terrainHeight, WORLD_HALF } from "./world.js";
...
export function createWildlife(seed = 112) {
  const random = seededRandom(seed);
  const rand = (min, max) => min + random() * (max - min);
  const worldHalf = WORLD_HALF;
  const randomInWorld = (pad) => rand(-worldHalf + pad, worldHalf - pad);
  const animals = [];
  function spawnAnimal(type, options) {
    const x = randomInWorld(options.pad);
    const z = randomInWorld(options.pad);
    const group = { position: { x, y: terrainHeight(x, z), z }, rotation: { y: random() * Math.PI * 2 }, visible: true };
  ...
  function update(delta) {
    animals.forEach((animal) => {
      ...
      animal.group.position.x = Math.max(-worldHalf, Math.min(worldHalf, animal.group.position.x));
      animal.group.position.z = Math.max(-worldHalf, Math.min(worldHalf, animal.group.position.z));
      ...
      if (scared) {
        ...
        animal.speed = animal.speed + (animal.baseSpeed - animal.speed) * Math.min(1, delta * 2);
      }
    });
  }

  return { animals, applyDamage, squirt, update };
}
